...
%% END MACROS SECTION
  ...
  Niklaus J. G"runwald\,\textsuperscript{1,3*}}
% \\
\def\Address{
  ...
  \def\corrAuthor{Niklaus J. G"runwald}\def\corrAddress{Horticultural Crops Research Laboratory USDA ARS\\3420 NW Orchard Ave.\\Corvallis, OR, 97330}\def\corrEmail{\href{mailto:grunwaln@science.oregonstate.edu}{\nolinkurl{grunwaln@science.oregonstate.edu}}}
...
introdcued the R package \emph{poppr} specifically developed for
analysis of clonal populations (Kamvar et al., 2014). \emph{Poppr}
prvisouly introduced several novel features including the ability to
conduct a hierarchical analysis across unlimited hierarchies, test for
linkage assocition, graph minimum spanning networks or provide boostrap
support for Bruvi's distance in resulting trees. In its first iteration
\emph{poppr} was appropriate fro tradiational markers systems, but not
ideally suited to population genomic data resulting from high throughput
sequencing methods.
...
are different. We introdcue a a new transparent method for collapsing
...
throughput sequencing, SNP data is now availabe in in a genome-wide
