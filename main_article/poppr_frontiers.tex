...
% I think it would be good to call our new MLG boundary algorithm something catchy
% like **clone caller** or **cloneR** or **MLG boundary** or **MLG SNPer**. Can't
% put my finger on it, but it is there somewhere hiding in plain sight ...
% **cloneBoundaryCaller**, **cloneCollapser**

>>>>>>> b2210c4d0ccd6567d51d0a4e95af132edf3cb510
...
To paraphrase XXX, nothing make sense in the field of plant-microbe
interactions in the absence of population genetics. Selection and drift
act on alleles in a population. Thus, a true understanding of how
pathogens evolve and adapt to crops and fungicides, can only emerge in
the context of population level phenomena given the genealogical or
demographic history of populations. The field of population genetics, in
the era of whole genome resequencing, provides unprecedented power to
describe the evolutionary history and population processes that drive
coevolution between pathogens and hosts. This powerful field thus
critically enables effective deployment of R genes, design of pathogen
informed plant resistance breeding programs, and implementation of
fungicide rotations that minimize emergence of resistance.
...
underlying the theory are violated. Yet, most plant pathogen populations
are at least partially clonal if not completely clonal. Thus,
...
polymorphisms (SNPs) in populations. Availability of these large SNP
data sets provides new challenges for data analysis. For example, it is
not clear what a clone is in large SNP data where the chance of
observing variation at a given SNP locus within independent samples of
the same clone are substantial enough that novel tools for definition of
clone boundaries are required. With traditional marker data (e.g., SSR,
AFLP) a clone was typically defined as a unique multilocus genotype
(MLG). However, with large SNP data a measure of genetic distance is
required to define the boundary of an MLG (e.g., clone) or the
boundaries of a clonal lineage. Definition of a clone is further
complicated by the presence of missing data that is typical for reduced
representation libraries used in GBS or genome resequencing. Are two
individuals that are identical for all observed SNPs different if one
additional locus has missing data?

Another major issues with large SNP data is computational efficiency.
Analyses that resample a data set, are iterative of rely on MCMC are
computationally slow for large data and thus are preferentially
implemented in a low level language such as C rather than in interpreted
languages such as R.
...
\subsubsection*{\emph{P. infestans}}\label{p.-infestans}
...
\subsubsection*{\emph{P. infestans}}\label{p.-infestans-1}
\addcontentsline{toc}{subsubsection}{\emph{P. infestans}}
