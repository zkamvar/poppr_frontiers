...
understanding plant microbe interactions, adaptation of pathogens to hsots, reactive breeding and deployment of R genes and design of infromed management strategies. With the advent of high throughput sequencing technologies, obtaining genomic sequences for
...
The research community using the R statistical and computing language [@R] has developed a plethora of new resources for population genetic analysis [@pegas; @Jombart_2008]. Recently, we introdcued the R package *poppr* specifically developed for analysis of clonal populations [@kamvar2014poppr]. *Poppr* prvisouly introduced several novel features including the ability to conduct a hierarchical analysis across unlimited hierarchies, test for linkage assocition, graph minimum spanning networks or provide boostrap support for Bruvi's distance in resulting trees. In its first iteration *poppr* was appropriate fro tradiational markers systems, but not ideally suited to population genomic data resulting from high throughput sequencing methods. 
...
just comparing what multilocus genotypes are different. We introdcue a a new transparent method for collapsing unique multilocus genotypes determined by naive string
