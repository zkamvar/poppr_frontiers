...
runtitle: "Novel R tools for population genomics"
author:
  - name: Zhian N. Kamvar
    affiliation: '1'
    etal: Kamvar
...
fungicides, or other factors, can only be elucidated in the context of population level
...
@Jombart_2008]. R is particularly appealing because all code is open source and functions can be evaluated and modified by any user. Recently, we introduced the R package *poppr* specifically
...
leukemia of clams [@metzger2015horizontal], study of the vector-mediated parent-to-offspring transmission in an avian malaria-like parasite
[@chakarov2015apparent], and characterization of the emergence of the invasive
forest pathogen *Hymenoscyphus pseudoalbidus* [@gross2014population]. It has also been used to implement real-time, online shinyR based tools for visualizing relationships among unknown MLGs in reference databases  (http://phytophthora-id.org/) [@grunwald2011phytophthora]
<!-- 
...
population genetic analysis of organisms that are known
...
error is also greatly increased and missing data is frequent [@mastretta2015restriction]. Taking this fact
...
MLGs are different. We introduce a new method for collapsing
...
in the mid 19th century [@goss2014irish; @yoshida2013rise]. *P.
...
markers [@lees2006novel; @li2013efficient]. For these data, we used `mlg.filter` to detect all of
...
We utilized simulated data to evaluate the effect of sequencing error and missing data on MLG calling. We constructed the data using the `glSim` function in *adegenet*
...
(Fig. 3). Note that the reticulated network (Fig. 3B) showed patterns corresponding with those resulting from a discriminant analysis of principal components (Fig. 3D) [@jombart2010discriminant].
...
being able to use R's matrix algebra capabilities to efficiently calculate genetic
...
*adegenet*'s "genind" object (Kamvar and Gr\"unwald, unpublished). The genclone object formalized the definitions of
multilocus genotypes and population hierarchies by adding two slots called "mlg"
and "hierarchy" that carried a numeric vector and a data frame, respectively.
These new slots allow for increased efficiency and ease of use by allowing these
...
The functionality represented by the hierarchy slot has now been migrated from the  *poppr* to the
...
upgrade following the 2015 R hackathon on population genetics i(https://github.com/NESCent/r-popgen-hackathon) and have not yet been updated in CRAN. We will upload *poppr* 2.0 to CRAN once all other reverse dependent packages have been updated.
...
<!-- anybody else to thank? -->
We thank Ignazio Carbone for discussions on the index of association and Thibaut Jombart for allowing us to incorporate the `strata` slot anr relted methds in *adegenet*. We also thank all the members of the 2015 R hackathon on population genetics in Durham, NC for their advice and input (https://github.com/NESCent/r-popgen-hackathon). This work was supported in part by US Department of Agriculture (USDA) Agricultural Research Service Grant 5358-22000-039-00D, USDA APHIS, the USDA-ARS Floriculture Nursery Initiative, and the USDA-Forest Service Forest Health Monitoring Program.
...
**Table 1** Contingency table comparing multilocus lineages assigned based on average
