...
nearest neighbor, and UPGMA (average neighbor) [citation needed].
...
separate clonal lineages using a combination of various molecular methods
including AFLP and microsatellite markers [cooke]. For these data, we used
`mlg.filter` to detect all of the distance thresholds at which 18 multilocus
lineages would be resolved. We the used these thresholds to define multilocus
lineages and create contingency tables and dendrograms to determine how well the
multilocus lineages were detected.
...
column represents a separate hierarchical level. The user could then define the
population factor for the data by providing a hierarchical formula that would
contain one or more levels from the hierarchy slot to be combined.
...
clonal data sets: *Phytophthora ramorum* data from Nurseries and Oregon forests,
and H3N2 flu virus data from the `adegenet` package using years as the
population factor[@kamvar2014sudden;
@jombart2010discriminant]. Minimum spanning networks were created with and
without reticulation using the *poppr* functions `bruvo.msn` and `diss.dist` for
the *P. ramorum* and H3N2 data, respectively [@kamvar2014poppr;
@bruvo2004simple]. To detect mlg clusters, the infoMAP community detection
algorithm was applied with 10,000 trials as implemented in the R package
`igraph` version 0.7.1 utilizing genetic distance as edge weights and number of
samples in each MLG as vertex weights [@csardi2006igraph; @rosvall2008maps].
...
The un-reticulated minimum spanning tree revealed 20 communities with a maximum
community size of 7. In the ability to predict Hunter Creek as belonging to a
single community, the reticulated network was successful whereas the minimum
spanning tree separated one genotype from that community. The entropy for the
reticulated network was lower for all populations except for the Coast
population. 


the infoMAP algorithm revealed 62 communities with a maximum community size of
80 for the reticulate network of the H3N2 data and 117 communities with a
maximum community size of 24 for the minimum spanning tree. The entropy across
years was greatly decreased for all populations with the reticulate network
