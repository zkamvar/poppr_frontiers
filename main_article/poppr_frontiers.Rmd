...
the simulations, two diploid data sets were created, each with 10k SNPs and 200
samples with 10 ancestral populations of even sizes. These populations were
randomly "mated" by first simulating meiotic crossover and then randomly
choosing one chromosome from two parents at random and then adding mutations to
each at a rate $\mu = 5e^{-3}$. Clones were created in one data set by randomly
sampling the data with replacement in half of the twenty populations and
subsequently marked with unique identifiers. It is well documented that reduced-
representation sequencing can introduce several erroneous calls and missing data
[@mastretta2015restriction]. To reflect this, we mutated SNPs at a rate of 5%
and inserted an average of 21% missing data for each sample after clones were
created, ensuring that no two sequences were alike. After pooling, the 20% of
the data set was randomly sampled for analysis. Genetic distance was represented
as the fraction of different sites between samples, counting missing data as
equivalent in comparison. Since each data set would have varying distances
between samples, the clonal boundary threshold was defined as the midpoint of
the largest gap between two thresholds that collapsed less than 50% of the data.
